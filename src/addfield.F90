...
subroutine addfield(cgrib, lcgrib, ipdsnum, ipdstmpl, ipdstmplen, &
...
#ifdef LOGGING
  print *, 'addfield lcgrib ', lcgrib, ' ipdsnum ', ipdsnum, ' ipdstmplen ', ipdstmplen, &
       ' numcoord ', numcoord, ' idrsnum ', idrsnum, ' idrstmplen ', idrstmplen, &
       ' ngrdpts ', ngrdpts, ' ibmap ', ibmap, ' fld(1) ', fld(1)
#endif
  ...
  ! Extend the Product Definition Template, if necessary. The number
...
#ifdef LOGGING
  print *, 'mapdrslen ', mapdrslen, ' needext ', needext
#endif
  ...
  !     Pack up each input value in array idrstmpl into the the
  !     appropriate number of octets, which are specified in corresponding
  ...
  ! Calculate length of section 5 and store it in octets 1-4 of
  ! section 5.
  ...
  ! Calculate length of section 7 and store it in octets 1-4 of
  ! section 7.
