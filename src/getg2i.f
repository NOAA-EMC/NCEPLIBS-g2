C> @file
C> @brief Read a GRIB2 index file and return its contents.
C> @author Mark Iredell @date 1995-10-31

C> Read a GRIB2 index file and return its contents.
C>
C> The index file may be generated by the grb2index utility of the
C> [NCEPLIBS-grib_util](https://github.com/NOAA-EMC/NCEPLIBS-grib_util)
C> project.
C>
C> The index file has two header records:
C> 1. an 81-byte header with 'GB2IX1' in columns 42-47
C> 2. an 81-byte header with number of bytes to skip before index
C> records, total length in bytes of the index records, number of
C> index records, and GRIB file basename written in format
C> ('IX1FORM:',3i10,2x,a40).
C>
C> Each record in the index table contains the following fields. All
C> integers are in big-endian format in the file.
C>
C> - byte 001 - 004 length of index record
C> - byte 005 - 008 bytes to skip in data file before grib message
C> - byte 009 - 012 bytes to skip in message before lus (local use) set = 0, if no local section.
...
C> @param[out] cbuf Pointer to a buffer that will get the index
C> records. Memory will be allocated within this function, so callers
C> must free the memory that cbuf points to, using deallocate(cbuf)
C> when cbuf is no longer needed.
