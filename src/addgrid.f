...
!
      ...
      call g2_sbyte(cgrib,three,iofst,8)     ! Store section number ( 3 )
      iofst=iofst+8
      call g2_sbyte(cgrib,igds(1),iofst,8)     ! Store source of Grid def.
      iofst=iofst+8
      call g2_sbyte(cgrib,igds(2),iofst,32)    ! Store number of data pts.
      iofst=iofst+32
      call g2_sbyte(cgrib,igds(3),iofst,8)     ! Store number of extra octets.
      iofst=iofst+8
      call g2_sbyte(cgrib,igds(4),iofst,8)     ! Store interp. of extra octets.
      iofst=iofst+8
      !   if Octet 6 is not equal to zero, Grid Definition Template may
      !   not be supplied.
      if ( igds(1).eq.0 ) then
        call g2_sbyte(cgrib,igds(5),iofst,16)  ! Store Grid Def Template num.
      else
        call g2_sbyte(cgrib,65535,iofst,16)   ! Store missing value as Grid Def Template num.
      endif
      iofst=iofst+16
      !
     ...
     &                       iret)
      ...
      do i=1,mapgridlen
        ...
        if ( (mapgrid(i).ge.0).or.(igdstmpl(i).ge.0) ) then
          call g2_sbyte(cgrib,igdstmpl(i),iofst,nbits)
        else
          call g2_sbyte(cgrib,one,iofst,1)
          call g2_sbyte(cgrib,iabs(igdstmpl(i)),iofst+1,nbits-1)
        endif
        iofst=iofst+nbits
      enddo
      !
      ...
      if ( igds(3).ne.0 ) then
         nbits=igds(3)*8
         call g2_sbytes(cgrib,ideflist,iofst,nbits,0,idefnum)
      ...
      call g2_sbyte(cgrib,lensec3,ibeg,32)

!
!  Update current byte total of message in Section 0
!
      call g2_sbyte(cgrib,lencurr+lensec3,96,32)
