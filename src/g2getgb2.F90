...
!> Find and extract a GRIB2 message from a file with index version 1
!> or 2.
!>
...
!> - 99 Request not found.
!>
!> @author Mark Iredell, Edward Hartnett @date 1994-04-01
subroutine getgb2p2(lugb, lugi, idxver, j, jdisc, jids, jpdtn, jpdt, jgdtn, jgdt,  &
  ...
  integer, intent(in) :: j, jdisc, jpdtn, jgdtn
  ...
  nullify(gribm)
  call getgb2rp2(lugb, idxver, cbuf(lpos:), extract, gribm, leng, iret)

  k = jk

  ! Free cbuf memory allocated in getg2i/getg2i2r().
  if (associated(cbuf)) deallocate(cbuf)
  
  call gf_free(gfld)
end subroutine getgb2p2

!> Find and extract a GRIB2 message from a file, using index version
!> 1. Use getgb2p2() to use either index version 1 or version 2.
!>
!> This subroutine reads a GRIB index file (or optionally the GRIB
!> file itself) to get the index buffer (i.e. table of contents) for
!> the GRIB file. It finds in the index buffer a reference to the
...
!> - 99 Request not found.
!>
!> @author Edward Hartnett @date 2024-04-22
subroutine getgb2p(lugb, lugi, j, jdisc, jids, jpdtn, jpdt, jgdtn, jgdt,  &
  ...
  idxver = 1
  call getgb2p2(lugb, lugi, idxver, j, jdisc, jids, jpdtn, jpdt, jgdtn, jgdt,  &
          extract, k, gribm, leng, iret)
