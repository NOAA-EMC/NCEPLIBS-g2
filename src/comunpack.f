...
!
      if (idrsnum.eq.3) then
         if (nbitsd.ne.0) then
              call g2_gbyte(cpack,ival1,iofst,nbitsd)
              iofst=iofst+nbitsd
              if (idrstmpl(17).eq.2) then
                 call g2_gbyte(cpack,ival2,iofst,nbitsd)
                 iofst=iofst+nbitsd
              endif
              call g2_gbyte(cpack,isign,iofst,1)
              iofst=iofst+1
              call g2_gbyte(cpack,minsd,iofst,nbitsd-1)
...
!
      ...
      if (nbitsgref.ne.0) then
         call g2_gbytes(cpack,gref,iofst,nbitsgref,0,ngroups)
...
!
      ...
      if (nbitsgwidth.ne.0) then
         call g2_gbytes(cpack,gwidth,iofst,nbitsgwidth,0,ngroups)
...
!
      ...
      if (nbitsglen.ne.0) then
         call g2_gbytes(cpack,glen,iofst,nbitsglen,0,ngroups)
      ...
      if ( idrstmpl(7).eq.0 ) then        ! no missing values
         ...
         !write(78,*)'NGP ',j,gwidth(j),glen(j),gref(j)
           if (gwidth(j).ne.0) then
             call g2_gbytes(cpack,ifld(n),iofst,gwidth(j),0,glen(j))
             do k=1,glen(j)
